...
void* threadpool_manager(void* arg)
{
    ...
    while (!pool->shutdown)
    {
        ...
        // 添加线程
        // 管理者线程判断是否需要创建线程
        if (liveNum<pool->maxThreadNum && queueSize+busyNum>liveNum)
        {
            // 加锁
            pthread_mutex_lock(&pool->poolMutex);
            int count=0;
            // 创建线程
            for (int i = 0; i < pool->maxThreadNum  && pool->liveThreadNum<pool->maxThreadNum && queueSize+busyNum>liveNum && count<NUM; i++)
            {
